...
using (var scope = app.Services.CreateScope())
{
    ...
    if (!isSqliteProvider)
    {
        ...
        TryExecuteSqlStatements(db,
            ...
            "ALTER TABLE \"Creatures\" ADD COLUMN IF NOT EXISTS \"BonusActions\" text NULL;",
            "ALTER TABLE \"Creatures\" ADD COLUMN IF NOT EXISTS \"Reactions\" text NULL;",
        ...
        db.Database.ExecuteSqlRaw("""
            ...
            CREATE TABLE IF NOT EXISTS "CreatureBonusActions" ("CreatureBonusActionId" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "CreatureId" integer NOT NULL, "Name" text NOT NULL, "Description" text NULL, "SortOrder" integer NOT NULL DEFAULT 0);
            CREATE INDEX IF NOT EXISTS "IX_CreatureBonusActions_CreatureId_SortOrder" ON "CreatureBonusActions" ("CreatureId", "SortOrder");
            CREATE TABLE IF NOT EXISTS "CreatureReactions" ("CreatureReactionId" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "CreatureId" integer NOT NULL, "Name" text NOT NULL, "Description" text NULL, "SortOrder" integer NOT NULL DEFAULT 0);
            CREATE INDEX IF NOT EXISTS "IX_CreatureReactions_CreatureId_SortOrder" ON "CreatureReactions" ("CreatureId", "SortOrder");
    ...
    db.Database.ExecuteSqlRaw("""
        ...
        CREATE TABLE IF NOT EXISTS CreatureBonusActions (
            CreatureBonusActionId INTEGER NOT NULL CONSTRAINT PK_CreatureBonusActions PRIMARY KEY AUTOINCREMENT,
            CreatureId INTEGER NOT NULL,
            Name TEXT NOT NULL,
            Description TEXT NULL,
            SortOrder INTEGER NOT NULL DEFAULT 0,
            CONSTRAINT FK_CreatureBonusActions_Creatures_CreatureId FOREIGN KEY (CreatureId) REFERENCES Creatures (CreatureId) ON DELETE CASCADE
        );
        CREATE INDEX IF NOT EXISTS IX_CreatureBonusActions_CreatureId_SortOrder ON CreatureBonusActions (CreatureId, SortOrder);

        CREATE TABLE IF NOT EXISTS CreatureReactions (
            CreatureReactionId INTEGER NOT NULL CONSTRAINT PK_CreatureReactions PRIMARY KEY AUTOINCREMENT,
            CreatureId INTEGER NOT NULL,
            Name TEXT NOT NULL,
            Description TEXT NULL,
            SortOrder INTEGER NOT NULL DEFAULT 0,
            CONSTRAINT FK_CreatureReactions_Creatures_CreatureId FOREIGN KEY (CreatureId) REFERENCES Creatures (CreatureId) ON DELETE CASCADE
        );
        CREATE INDEX IF NOT EXISTS IX_CreatureReactions_CreatureId_SortOrder ON CreatureReactions (CreatureId, SortOrder);
    """);
    ...
    TryExecuteSqlStatements(db,
        ...
        "ALTER TABLE Creatures ADD COLUMN BonusActions TEXT NULL;",
        "ALTER TABLE Creatures ADD COLUMN Reactions TEXT NULL;",
        "CREATE TABLE IF NOT EXISTS CreatureBonusActions (CreatureBonusActionId INTEGER NOT NULL CONSTRAINT PK_CreatureBonusActions PRIMARY KEY AUTOINCREMENT, CreatureId INTEGER NOT NULL, Name TEXT NOT NULL, Description TEXT NULL, SortOrder INTEGER NOT NULL DEFAULT 0, CONSTRAINT FK_CreatureBonusActions_Creatures_CreatureId FOREIGN KEY (CreatureId) REFERENCES Creatures (CreatureId) ON DELETE CASCADE);",
        "CREATE INDEX IF NOT EXISTS IX_CreatureBonusActions_CreatureId_SortOrder ON CreatureBonusActions (CreatureId, SortOrder);",
        "CREATE TABLE IF NOT EXISTS CreatureReactions (CreatureReactionId INTEGER NOT NULL CONSTRAINT PK_CreatureReactions PRIMARY KEY AUTOINCREMENT, CreatureId INTEGER NOT NULL, Name TEXT NOT NULL, Description TEXT NULL, SortOrder INTEGER NOT NULL DEFAULT 0, CONSTRAINT FK_CreatureReactions_Creatures_CreatureId FOREIGN KEY (CreatureId) REFERENCES Creatures (CreatureId) ON DELETE CASCADE);",
        "CREATE INDEX IF NOT EXISTS IX_CreatureReactions_CreatureId_SortOrder ON CreatureReactions (CreatureId, SortOrder);",
...
app.MapGet("/api/creatures", async (HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    var rows = await db.Creatures
        ...
        .Include(x => x.BonusActionList)
        .Include(x => x.ReactionList)
...
app.MapPost("/api/creatures/{id:int}/clone", async (int id, HttpContext http, AppDbContext db) =>
{
    ...
    foreach (var bonusAction in row.BonusActionList.OrderBy(x => x.SortOrder))
        clone.BonusActionList.Add(new CreatureBonusAction { Name = bonusAction.Name, Description = bonusAction.Description, SortOrder = bonusAction.SortOrder });
    foreach (var reaction in row.ReactionList.OrderBy(x => x.SortOrder))
        clone.ReactionList.Add(new CreatureReaction { Name = reaction.Name, Description = reaction.Description, SortOrder = reaction.SortOrder });
...
static CreatureResponse ToCreatureResponse(Creature row, string? ownerUsername, bool ownerIsAdmin, bool userCanEdit)
{
    ...
    return new CreatureResponse
    {
        ...
        BonusActions = BuildCreatureEntryDtos(row.BonusActionList, row.BonusActions),
        Reactions = BuildCreatureEntryDtos(row.ReactionList, row.Reactions),
        Strength = row.Strength,
        Dexterity = row.Dexterity,
        Constitution = row.Constitution,
        Intelligence = row.Intelligence,
...
static List<CreatureEntryDto> BuildCreatureEntryDtos<T>(IEnumerable<T> rows, string? legacyText) where T : class
{
    var items = rows.Select(x => x switch
    {
        ...
        CreatureBonusAction bonusAction => new CreatureEntryDto { Name = bonusAction.Name, Description = bonusAction.Description, SortOrder = bonusAction.SortOrder },
        CreatureReaction reaction => new CreatureEntryDto { Name = reaction.Name, Description = reaction.Description, SortOrder = reaction.SortOrder },
...
static void ApplyCreatureEntries(Creature row, UpsertCreatureRequest req)
{
    ...
    row.BonusActionList.Clear();
    foreach (var entry in NormalizeCreatureEntries(req.BonusActions))
        row.BonusActionList.Add(new CreatureBonusAction { Name = entry.Name, Description = entry.Description, SortOrder = entry.SortOrder });

    row.ReactionList.Clear();
    foreach (var entry in NormalizeCreatureEntries(req.Reactions))
        row.ReactionList.Add(new CreatureReaction { Name = entry.Name, Description = entry.Description, SortOrder = entry.SortOrder });
}
