...
app.MapGet("/api/items", async (HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    var rows = await db.Items
        .Where(x => x.DateDeletedUtc == null && (includeAll || x.OwnerAppUserId == userId.Value || sharedIds.Contains(x.ItemId)))
        .OrderBy(x => x.Name)
        .Select(x => new ItemResponse
        {
            ItemId = x.ItemId, OwnerAppUserId = x.OwnerAppUserId, Name = x.Name, Description = x.Description, ItemType = (int)x.ItemType, Rarity = (int)x.Rarity,
...
app.MapGet("/api/items/{id:int}", async (int id, HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    var canView = includeAll || row.OwnerAppUserId == userId.Value || await db.ItemShares.AnyAsync(x => x.ItemId == id && x.SharedWithUserId == userId.Value);
    if (!canView) return Results.Forbid();

    return Results.Ok(new ItemResponse
    {
        ItemId = row.ItemId, OwnerAppUserId = row.OwnerAppUserId, Name = row.Name, Description = row.Description, ItemType = (int)row.ItemType, Rarity = (int)row.Rarity,
...
app.MapGet("/api/campaigns", async (HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    var rows = await db.Campaigns.Where(x => x.DateDeletedUtc == null && (includeAll || x.OwnerAppUserId == userId.Value || sharedIds.Contains(x.CampaignId))).OrderBy(x => x.Name)
        .Select(x => new CampaignResponse { CampaignId = x.CampaignId, OwnerAppUserId = x.OwnerAppUserId, Name = x.Name, Description = x.Description }).ToListAsync();
    return Results.Ok(rows);
}).RequireAuthorization();
...
app.MapGet("/api/campaigns/{id:int}", async (int id, HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    if (!canView) return Results.Forbid();
    return Results.Ok(new CampaignResponse { CampaignId = row.CampaignId, OwnerAppUserId = row.OwnerAppUserId, Name = row.Name, Description = row.Description });
}).RequireAuthorization();
...
app.MapGet("/api/parties", async (HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    var rows = await db.Parties.Where(x => x.DateDeletedUtc == null && (includeAll || x.OwnerAppUserId == userId.Value || sharedIds.Contains(x.PartyId))).OrderBy(x => x.Name)
        .Select(x => new PartyResponse { PartyId = x.PartyId, OwnerAppUserId = x.OwnerAppUserId, Name = x.Name, Description = x.Description, CampaignId = x.CampaignId == 0 ? null : x.CampaignId, MemberCount = db.Characters.Count(c => c.DateDeletedUtc == null && c.PartyId == x.PartyId && c.CharacterType == CharacterType.PC) }).ToListAsync();
    return Results.Ok(rows);
}).RequireAuthorization();
...
app.MapGet("/api/parties/{id:int}", async (int id, HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    if (!canView) return Results.Forbid();
    var members = await db.Characters.CountAsync(c => c.DateDeletedUtc == null && c.PartyId == id && c.CharacterType == CharacterType.PC);
    return Results.Ok(new PartyResponse { PartyId = row.PartyId, OwnerAppUserId = row.OwnerAppUserId, Name = row.Name, Description = row.Description, CampaignId = row.CampaignId == 0 ? null : row.CampaignId, MemberCount = members });
}).RequireAuthorization();
...
app.MapPut("/api/parties/{id:int}", async (int id, UpsertPartyRequest req, HttpContext http, AppDbContext db) =>
{
    ...
    if (!canEdit && !IsAdmin(http)) return Results.Forbid();
    row.Name = TitleNormalization.ToPascalTitle(req.Name); row.Description = string.IsNullOrWhiteSpace(req.Description) ? null : req.Description.Trim(); row.CampaignId = req.CampaignId ?? 0; row.DateModifiedUtc = DateTime.UtcNow;
    await db.SaveChangesAsync(); return Results.Ok(new { row.PartyId });
}).RequireAuthorization();
...
app.MapGet("/api/parties/{id:int}/members", async (int id, HttpContext http, AppDbContext db, bool? showAll) =>
{
    var userId = GetUserId(http); if (userId is null) return Results.Unauthorized();
    var includeAll = IsAdmin(http) && showAll == true;
    var party = await db.Parties.FirstOrDefaultAsync(x => x.PartyId == id && x.DateDeletedUtc == null); if (party is null) return Results.NotFound();
    var canView = includeAll || party.OwnerAppUserId == userId.Value || await db.PartyShares.AnyAsync(x => x.PartyId == id && x.SharedWithUserId == userId.Value);
    if (!canView) return Results.Forbid();
    var rows = await db.Characters.Where(x => x.DateDeletedUtc == null && x.CharacterType == CharacterType.PC && x.PartyId == id).OrderBy(x => x.Name)
        .Select(x => new { x.CharacterId, x.Name, x.ClassName, x.Level, x.ArmorClass, x.HitPointsCurrent, x.HitPointsMax }).ToListAsync();
    return Results.Ok(rows);
}).RequireAuthorization();
...
app.MapPost("/api/parties/{id:int}/share", async (int id, ShareRecordRequest req, HttpContext http, AppDbContext db) =>
{
    ...
    if (link is null) db.PartyShares.Add(new PartyShare { PartyId = id, SharedWithUserId = req.UserId, Permission = (SharePermission)req.Permission });
    else link.Permission = (SharePermission)req.Permission;
    await db.SaveChangesAsync(); return Results.Ok();
}).RequireAuthorization();
...
app.MapDelete("/api/parties/{id:int}/share/{userId:int}", async (int id, int userId, HttpContext http, AppDbContext db) =>
{
    ...
    var row = await db.Parties.FirstOrDefaultAsync(x => x.PartyId == id && x.DateDeletedUtc == null); if (row is null) return Results.NotFound();
    if (row.OwnerAppUserId != me.Value && !IsAdmin(http)) return Results.Forbid();
    var link = await db.PartyShares.FirstOrDefaultAsync(x => x.PartyId == id && x.SharedWithUserId == userId); if (link is null) return Results.NotFound();
    db.PartyShares.Remove(link); await db.SaveChangesAsync(); return Results.NoContent();
}).RequireAuthorization();
...
app.MapGet("/api/quests", async (HttpContext http, AppDbContext db, bool? showAll) =>
{
    ...
    var rows = await db.Quests.Where(x => x.DateDeletedUtc == null && (includeAll || x.OwnerAppUserId == userId.Value || sharedIds.Contains(x.QuestId))).OrderBy(x => x.Title)
        .Select(x => new QuestResponse { QuestId = x.QuestId, OwnerAppUserId = x.OwnerAppUserId, CampaignId = x.CampaignId == 0 ? null : x.CampaignId, Title = x.Title, Summary = x.Summary, Mode = (int)x.Mode, UseChoiceMode = x.UseChoiceMode, StartNodeId = x.StartNodeId }).ToListAsync();
    return Results.Ok(rows);
}).RequireAuthorization();
...
app.MapGet("/api/quests/{id:int}", async (int id, HttpContext http, AppDbContext db, bool? showAll) =>
{
    var userId = GetUserId(http); if (userId is null) return Results.Unauthorized();
    var includeAll = IsAdmin(http) && showAll == true;
    var quest = await db.Quests.FirstOrDefaultAsync(x => x.QuestId == id && x.DateDeletedUtc == null); if (quest is null) return Results.NotFound();
    var canView = includeAll || quest.OwnerAppUserId == userId.Value || await db.QuestShares.AnyAsync(x => x.QuestId == id && x.SharedWithUserId == userId.Value);
    if (!canView) return Results.Forbid();
    var nodes = await db.QuestNodes.Where(x => x.QuestId == id && x.DateDeletedUtc == null).OrderBy(x => x.OrderIndex).Select(x => new QuestNodeResponse { QuestNodeId = x.QuestNodeId, Title = x.Title, NodeType = (int)x.NodeType, OrderIndex = x.OrderIndex, BodyMarkdown = x.BodyMarkdown, DmHints = x.DmHints, EncounterId = x.EncounterId, CanvasX = x.CanvasX, CanvasY = x.CanvasY }).ToListAsync();
    var choices = await db.QuestChoices.Where(x => x.QuestId == id && x.DateDeletedUtc == null).OrderBy(x => x.FromNodeId).ThenBy(x => x.OrderIndex).Select(x => new QuestChoiceResponse { QuestChoiceId = x.QuestChoiceId, FromNodeId = x.FromNodeId, ToNodeId = x.ToNodeId, Label = x.Label, ConditionExpression = x.ConditionExpression, EffectsJson = x.EffectsJson, OrderIndex = x.OrderIndex }).ToListAsync();
    return Results.Ok(new QuestResponse { QuestId = quest.QuestId, OwnerAppUserId = quest.OwnerAppUserId, CampaignId = quest.CampaignId == 0 ? null : quest.CampaignId, Title = quest.Title, Summary = quest.Summary, Mode = (int)quest.Mode, UseChoiceMode = quest.UseChoiceMode, StartNodeId = quest.StartNodeId, Nodes = nodes, Choices = choices });
}).RequireAuthorization();
...
app.MapPost("/api/quests/{id:int}/share", async (int id, ShareRecordRequest req, HttpContext http, AppDbContext db) =>
{
    ...
    if (link is null) db.QuestShares.Add(new QuestShare { QuestId = id, SharedWithUserId = req.UserId, Permission = (SharePermission)req.Permission });
    else link.Permission = (SharePermission)req.Permission;
    await db.SaveChangesAsync(); return Results.Ok();
}).RequireAuthorization();
...
app.MapDelete("/api/quests/{id:int}/share/{userId:int}", async (int id, int userId, HttpContext http, AppDbContext db) =>
{
    ...
    var row = await db.Quests.FirstOrDefaultAsync(x => x.QuestId == id && x.DateDeletedUtc == null); if (row is null) return Results.NotFound();
    if (row.OwnerAppUserId != me.Value && !IsAdmin(http)) return Results.Forbid();
    var link = await db.QuestShares.FirstOrDefaultAsync(x => x.QuestId == id && x.SharedWithUserId == userId); if (link is null) return Results.NotFound();
    db.QuestShares.Remove(link); await db.SaveChangesAsync(); return Results.NoContent();
}).RequireAuthorization();
...
app.MapPost("/api/quests/{questId:int}/nodes", async (int questId, UpsertQuestNodeRequest req, HttpContext http, AppDbContext db) =>
{
    ...
    if (!canEditQuest && !IsAdmin(http)) return Results.Forbid();
...
app.MapPost("/api/quests/{questId:int}/nodes/{nodeId:int}/position", async (int questId, int nodeId, System.Text.Json.JsonElement req, HttpContext http, AppDbContext db) =>
{
    var row = await db.QuestNodes.FirstOrDefaultAsync(x => x.QuestId == questId && x.QuestNodeId == nodeId && x.DateDeletedUtc == null);
    if (row is null) return Results.NotFound();
